		...
		while (client == null) {
			...
			if (entry != null) {
				...
				if (entry instanceof NettyPartitionRequestClient) {
					...
					//clients一开始都是装的ConnectingChannel，第二次访问的时候，会把里面的
					//partitionclient拿出来然后替换掉
				...
				//这是一个监听器，监听连接，当连接完成后，如果连接成功，监听器会调用
				//handleChannel，因为ChannelFuture可以获得channel，所以该对象就用这个channel
				//创建NettyClienthandler 和partitionClient，client就用这个channel写
				...
				if (old == null) {
					//在这里总客户端接到连接后，直接扔给监听器
					nettyClient.connect(connectionId.getAddress()).addListener(connectingChannel);

					//由于在ConnectingChannel中已经创建了partitionclient，所以直接获取
