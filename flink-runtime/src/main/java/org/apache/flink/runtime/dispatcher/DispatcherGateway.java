...
public interface DispatcherGateway extends FencedRpcGateway<DispatcherId>, RestfulGateway {
	 ...
	 */
	//提交一个job
	//对于dispatcher来说，提交了job就立马就行构建executiongraph
	//该方法一般由DispatcherRestEndpoint的Handler（RetriverHandler处理）
	//的GatewayRetriver检索到该dispatcher
	//然后交给jobsubmithandler,他会根据穿过来的job，调用submitJob方法
