...
public class Task implements Runnable, TaskSlotPayload, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
	...
	private void doRun() {
		...
		try {
			...
			//这行代码非常重要
			//为task的ResultPartition和InputGate分配BufferPool，准确来说是LocalBufferPool
			//而NetworkBufferPool为整个TaskManager所有的，所有task共用一个NetworkBufferPool
			//点进去就知道这里的bufferPool并不是new出来的，而是通过一个函数对象创建出来的（类似函数式编程）

			//对于inputGates来说，先通过调用assignExclusiveSegments给InputGate的每个
			//inputchannel赋予专门的buffer，该buffer是由networkbufferpool提供的，准确来说
			//是提供availableSegments中的Segment，然后用segment生成networkBuffer
			// 使用bufferPoolFactory获取一个BufferPool，
			// 然后requestPartitions，请求数据

			setupPartitionsAndGates(consumableNotifyingPartitionWriters, inputGates);

			for (ResultPartitionWriter partitionWriter : consumableNotifyingPartitionWriters) {
				//这里往taskEventDispatcher注册partition，
				//其实在里面他是给每一个partition创建一个TaskEventHandler
				//从翻译上来看，TaskEventhandler管理了EventListener，可见他是一个发布器
				//所以他是可以发布事件，让监听器监听到
				//可以看到taskEventDispatcher集成了taskEventpublisher，他就是来发布事件，处理所有的partition
			...
			//翻译说，单个任务的KvState注册的帮助器。
			//那应该是有关task的state，稍后再说
			TaskKvStateRegistry kvStateRegistry = kvStateService.createKvStateTaskRegistry(jobId, getJobVertexId());

			//可以看到，这个类把task的重要组件和重要信息封装起来了，后面要传给真正地执行类AbstractInvokable
			Environment env = new RuntimeEnvironment(
				//整个工作的id
				jobId,
				vertexId,//顶点id（暂不清楚）
				//执行id（应该是Execution尝试的id，这个很牛逼呀，flink把执行任务当成一种尝试，失败了其实还可以再次启动）
				executionId,
				executionConfig,
				taskInfo, //任务的信息
				jobConfiguration, //job的配置
				taskConfiguration, //task的配置
				userCodeClassLoader,
				memoryManager,
				ioManager,
				broadcastVariableManager,
				taskStateManager, //该接口提供报告和检索任务状态的方法。（state管理）
				aggregateManager, //通过此接口可以访问临时的，命名的全局聚合
				accumulatorRegistry,
				kvStateRegistry,
				inputSplitProvider, //输入任务的拆分提供者。
			...
			//确保用户代码类加载器可通过线程本地访问。
			//我们在实例化可调用对象之前设置了正确的上下文类加载器，
			//以便可调用对象在整个生命周期内都可用。
			executingThread.setContextClassLoader(userCodeClassLoader);

			// now load and instantiate the task's invokable code
			//现在加载并实例化任务的可调用代码
			//nameOfInvokableClass是从jobmanager传过来的
	...
	private ClassLoader createUserCodeClassloader() throws Exception {
		...
		//ChildURLClassLoader，首先从URL加载，然后才从父级加载URL。
		//ParentURLClassLoader，它首先从父级加载，然后才从URL加载。
	...
	public void triggerCheckpointBarrier(
		...
		if (executionState == ExecutionState.RUNNING && invokable != null) {
			try {
				//肯定是执行StreamTask的triggerCheckpointAsync方法了
