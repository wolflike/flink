...
public class DefaultDispatcherResourceManagerComponentFactory implements DispatcherResourceManagerComponentFactory {
	...
	/**
	 * 可以haService提供了检索leader服务和选举leader服务.
	 */
	@Override
	public DispatcherResourceManagerComponent create(
		...
		try {
			//这两个类就是检索服务，只有start和stop两个方法
			//start主要是利用LeaderGatewayRetriever作为监听器
			//当获取到leader的地址之后，就开始通知LeaderGatewayRetriever构建gateway
			dispatcherLeaderRetrievalService = highAvailabilityServices.getDispatcherLeaderRetriever();

			resourceManagerRetrievalService = highAvailabilityServices.getResourceManagerLeaderRetriever();

			//通过这个类可以找到DispatcherGateway，当DispatcherGateway改变时
			//这个类实现了LeaderRetrievalListener监听器
			//那么说明监听器注册到注册中心去了，当leader地址改变时，
			//监听器会通知检索器（这里应该是注册中心持有这个监听器，但是是一个接口，通过远程调用实现通知）
			//检索器收到通知之后会把新的leader地址保存，然后根据地址新建一个leadergateway与之通信
			//这里的rpcService就是用来获取远程gateway的，因为rpcservice是actorsystem
			//这里的gateway并不是与远程连接获取的，而是通过动态代理获取的
			//首先LeaderGatewayRetriever保存了leader的地址（这个地址不是ip:port，而是actor协议地址）
			//通过这个地址从actorsystem拿到actorRef，actorRef可以拿到actor的地址和端口，
			//然后根据这个三个属性创建一个invocationhandler,如果是本地的gateway直接进行函数调用
			//如果是远程那就远程调用
			...
			//这里为DispatcherRestEndpoint创建线程池（为了与客户端、webui的交互嘛）
			...
			//这个webmonitorEndpoint就是DispatcherRestEndpoint
			//就是为了与client还有前端进行rest通信
			//可以看到DispatcherRestEndpoint包含了dispatcherGatewayRetriever和resourceManagerGatewayRetriever
			...
			//利用dispatcherLeaderProcessFactory
			//创建dispatcherRunner
			//在其构造函数中用leaderElectionService选举该dispatcherRunner
			//如果在注册中心中的leader是该dispatcherRunner
			//那么会调用dispatcherRunner的grantleadership
			//开始工作
			...
			//这里就开始检索leader，以及构建gateway
			//在这里开始监听zookeeper上resourceManager和dispatcher 的leader变更
			//一旦有变化，就会通知相应的retriever（也即是LeaderRetrieverListener）
