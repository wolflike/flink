...
public class ResultPartition implements ResultPartitionWriter, BufferPoolOwner {
	 ...
	 */
	@Override
	//可以看到partition也有生成bufferpool的步骤
	//对比一下与inputgate的bufferPoolfactory的不同
	//private final SupplierWithException<BufferPool, IOException> bufferPoolFactory;这是inputgate的
	//private final FunctionWithException<BufferPoolOwner, BufferPool, IOException> bufferPoolFactory;这是partition的
	//显然Supplier只有返回参数没有形参，Function有形参也有返回参数
	//partition的bufferPoolFactory的形参是BufferPoolOwner，返回参数依然是Bufferpool
	//顾名思义，BufferpoolOwner是bufferpool的拥有者，其实他的功能是释放buffer，即释放资源
	//依葫芦画瓢，这里的bufferPoolFactory应该就是在partitionFactory中创建的
	//可以发现最终创建bufferpool的逻辑和inputgate是一样的，只不过它拥有Bufferpoolowner（可以释放资源）
	...
	public void setup() throws IOException {
		...
		//这句代码说明partition是由partitionmanager管理的
		//可以点进去看看，发现就是把partition放进partitionmanager的registeredpartitions中
		//在看看这个ResultPartitionManager类，发现他是实现了ResultPartitionProvider接口
		//从名字上看，他应该是提供ResultPartition的入口
