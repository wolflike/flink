 ...
 */
//通道处理程序从生产者那里读取缓冲区响应或错误响应的消息，
//以写入和刷新生产者未声明的信用。
//<p>在新的基于网络信用的模式下使用。
//该类是专门处理上游partition的response
	//从该类的属性inputChannels来看，说明他是为多个RemoteInputChannel服务的
	//而该类是属于partitionclient类的，他们是一对一关系
	//所以说partitionclient也是服务多个RemoteInputChannel
...
class CreditBasedPartitionRequestClientHandler extends ChannelInboundHandlerAdapter implements NetworkClientHandler {
	...
	@Override
	//向上游partition发送credit，最终调用userEventTriggered
	 ...
	 */
	@Override
	//通过通知客户处理程序管道中可用的信用来触发。
	//<p>使输入通道入队，并且如果此输入通道是队列中的第一个通道，
	//则将触发此输入通道的写入和刷新未声明的信用。
	//最终notifyCreditAvailable调用这个方法
	...
	private void decodeMsg(Object msg) throws Throwable {
		...
		// ---- Buffer --------------------------------------------------------
		//从这里可以看出，从上游partition获取到的数据是BufferResponse
		//点进BufferResponse类发现有两个重要的属性ByteBuf、InputChannelId
		//ByteBuf是netty用来缓存数据的，所以数据应该在ByteBuf中
		// InputChannelId应该是标记请求的InputChannel，因为handler每次接到response
		//总要给某个具体的inputchannel吧
		if (msgClazz == NettyMessage.BufferResponse.class) {
			...
			if (inputChannel == null) {
				//如果没有对应的inputchannel，说明这个inputchannel挂了
				//这个时候应该释放掉这个数据
	...
	private void decodeBufferOrEvent(RemoteInputChannel inputChannel, NettyMessage.BufferResponse bufferOrEvent) throws Throwable {
		try {
			...
			if (bufferOrEvent.isBuffer()) {
				...
				//inputChannel从自己的AvailableBufferQueue bufferQueue中拿
				//inputChannel的bufferQueue在前面咱们知道是inputgate调用assignExclusiveSegments
				//从NetworkBufferpool中拿到的
				Buffer buffer = inputChannel.requestBuffer();
				if (buffer != null) {
					//把response中的数据写到inputchannel的buffer中
					nettyBuffer.readBytes(buffer.asByteBuf(), receivedSize);
					buffer.setCompressed(bufferOrEvent.isCompressed);

					//处理这个buffer（这个buffer现在有数据了）
					//很明显在bufferQueue中拿到的buffer，填充了数据肯定要放进receivedBuffers中
	 ...
	 */
	//这里是把credit组装成Addcredit，然后发给上游的正真逻辑
