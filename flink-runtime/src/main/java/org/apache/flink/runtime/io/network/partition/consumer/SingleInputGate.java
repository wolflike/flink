...
public class SingleInputGate extends InputGate {
	...
	@Override
	//这里有三部分很重要
	//第一是assignExclusiveSegments，他是为每个inputchannel从全局或者说networkbufferpool
	//中获取segment，然后组装成buffer，这部分的buffer自然是放进bufferQueue中的floatingBuffers
	//点进去看看就会知道其实就是在networkbufferpool中availableSegments中获取的
	//而networkbufferpool中的availableSegments中的segment是根据配置文件生成的，比如每个taskmanager分配多少内存（默认64M）
	//分成多少段（所以应该是2048段），每段多少大小（默认32K）
	//第二当然是为inputgate生成一个bufferpool，这个bufferpool为所有的inputchannel工作
	//可以看到是通过bufferPoolFactory函数式对象直接生成的，其实这个函数式对象是在SingleInputGate生成的时候，
	//SingleInputGateFactory生成直接传给SingleInputGate的，这个函数式主体是通过BufferPoolFactory创建的
	//我们知道BufferPoolFactory接口的实现就是NetworkBufferPool
	//而NetworkBufferPool是taskmanager的shuffleservice（忘了叫啥名字）创建的，说明networkBufferpool是属于taskmanager的
	//而最终是由NetworkBufferPool来创建LocalBufferpool，最后用一个set保存创建的LocalBufferpool
	public void setup() throws IOException, InterruptedException {
		checkState(this.bufferPool == null, "Bug in input gate setup logic: Already registered buffer pool.");
		// assign exclusive buffers to input channels directly and use the rest for floating buffers
		//这里执行之后，所有的inputchannel都有了floatingbuffer资源
		//其实这里直接是从networkbufferpool申请buffer
		//下面再创建Localbufferpool，那就是从Localbufferpool获取buffer了
		assignExclusiveSegments();

		//获取到bufferPool之后，就可以直接从Localbufferpool获取buffer
		//而不是再去Networkbufferpool中获取segments，而组装成buffer了
		BufferPool bufferPool = bufferPoolFactory.get();
		setBufferPool(bufferPool);

		//为Inputchannel请求buffer
		//所以这里的第三步不是请求buffer，因为buffer都可以从Localbufferpool和networkbufferpool获取到
		//这里应该就是单纯地从上游partitions要数据，数据要过来之后，把数据拷贝到各自inputchannel的receivedbuffers中
		requestPartitions();
	}
	...
	private Optional<InputWithData<InputChannel, BufferAndAvailability>> waitAndGetNextData(boolean blocking)
		...
		while (true) {
			...
			// notifications.
			//从inputchannel获取buffer（已经有数据的buffer，也就是从receivedBuffers中拿）
			Optional<BufferAndAvailability> result = inputChannel.get().getNextBuffer();

			//为什么从inputChannelsWithData获取的inputchannel又加进inputChannelsWithData了？？？？？？？？
			synchronized (inputChannelsWithData) {
	...
	//inputgate知道inputchannel不为空，想要干点事
	...
	//把带有数据，而且不为空的channel加到inputChannelsWithData队列中
	//可以发现flink用ArrayDeque比较多
	private void queueChannel(InputChannel channel) {
		...
		synchronized (inputChannelsWithData) {
			...
			if (availableChannels == 0) {
				//可以看到availableChannels是在add之前获取的，就是说在没添加之前队列是没有channel
				//而且在添加之后，唤醒所有获取channel的线程，这是啥操作？？？？？？？？？？？
				//inputChannelsWithData一个channel都没有的时候
				inputChannelsWithData.notifyAll();
				toNotify = availabilityHelper.getUnavailableToResetAvailable();
			}
		}
