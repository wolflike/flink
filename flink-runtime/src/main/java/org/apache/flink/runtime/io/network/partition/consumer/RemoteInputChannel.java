...
public class RemoteInputChannel extends InputChannel implements BufferRecycler, BufferListener {
	...
	@Nonnull
	//全局内存段提供者，用于请求和回收专用缓冲区（仅适用于基于信用的缓冲区）。
	//这个其实是NetworkBufferPool
	...
	@VisibleForTesting
	@Override
	//在上游的resultPartition的subpartition中请求什么呢？？？(反正不是buffer，buffer是个缓冲区)
	public void requestSubpartition(int subpartitionIndex) throws IOException, InterruptedException {
		if (partitionRequestClient == null) {
			// Create a client and request the partition
			try {
				//创建与上游partition交互的client，
				//注意，该inputchannel拥有的客户端和其他客户端一起合用一个NettyClient
				//这里的PartitionRequestClient通过NettyConnectionManager的PartitionRequestClientFactory创建
				//从参数看出partitionclient和connectId是一一对应的
				//所以这里的创建不一定是新建，根据id直接获取也可以
			...
			//使用客户端发送请求，当然这个客户端最终是交由NettyClient发送请求
	 ...
	 */
	//从生产者的缓冲区响应中接收积压。
	//如果可用缓冲区的数量少于backlog + initialCredit，
	//它将从缓冲区池中请求浮动缓冲区，然后将未通知的信用通知生产者。
	void onSenderBacklog(int backlog) throws IOException {
		...
		synchronized (bufferQueue) {
			...
			while (bufferQueue.getAvailableBufferSize() < numRequiredBuffers && !isWaitingForFloatingBuffers) {
				//这里就是满足了可用缓冲区的数量少于backlog + initialCredit
				//所以需要从缓冲区池中请求浮动缓冲区
				//inputgate首先获取自己的LocalBufferPool，
				//然后就去LocalBufferPool的availableMemorySegments中去拿Segment，组装成NetworkBuffer
				//有可能拿不到，那就从LocalBufferpool中的NetworkBufferPool（这是taskmanager共用的，也就是全局的）的
				//availableMemorySegments中拿Segment
				//如果拿到了，说明分配buffer成功
				//可以看到这里的buffer并没有从自身的bufferQueue中获取
				//我知道啥原因了-----------------------
				//这里主要是根据上游的partition的积压信息，然后判断inputchannel自己的bufferQueue空间够不够
				//如果不够需要再从Localbufferpool或者networkbufferpool中获取segments组装成buffer，然后放进bufferQueue中
				//直到numRequestedBuffers不大于bufferQueue大小为止
				Buffer buffer = inputGate.getBufferPool().requestBuffer();
				if (buffer != null) {
					//把这个在全局拿到的buffer放进floatingbuffers
					//不是应该放到exclusivebuffers中嘛？？？？？？？？？？
					bufferQueue.addFloatingBuffer(buffer);
					numRequestedBuffers++;
				} else if (inputGate.getBufferProvider().addBufferListener(this)) {
					//如果申请不到buffer，那就把当前inputchannel当成监听器注册到Localbufferpool中
					//等有的时候，会调用监听器的notifyBufferAvailable，然后把buffer放进inputchannel的bufferQueue中
		...
		//申请了资源之后就和上游partition说，你可以放心地把积压数据过发过来了
		if (numRequestedBuffers > 0 && unannouncedCredit.getAndAdd(numRequestedBuffers) == 0) {
			//这里应该是给上游partition回复unannouncedCredit的
			notifyCreditAvailable();
		}
	}
	...
	public void onBuffer(Buffer buffer, int sequenceNumber, int backlog) throws IOException {
		...
		try {
			...
			synchronized (receivedBuffers) {
				...
				//向这个队列加buffer
				//很明显从bufferQueue中拿可用的buffer资源
				//然后把填充数据的buffer加进receivedBuffers
			...
			if (wasEmpty) {
				//如果receivedBuffers中还有buffer，那就应该通知inputchannel不为空
				//然后把该inputchannel加入到inputgate的inputChannelsWithData中
				//让task计算的时候使用
				notifyChannelNonEmpty();
			}

			if (backlog >= 0) {
				//对于上游partition的积压作出反应
				onSenderBacklog(backlog);
			}
		} finally {
			if (recycleBuffer) {
				//回收buffer，最终的buffer回收时放进Localbufferpool
				//而不是回收到NetworkBufferPool，这个是不可逆的
				buffer.recycleBuffer();
			}
		}
	}
