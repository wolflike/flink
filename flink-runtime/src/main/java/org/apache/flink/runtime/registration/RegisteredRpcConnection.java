 ...
 */
//该实用工具类实现了从一个组件到另一组件的RPC连接的基础，例如，从TaskExecutor到ResourceManager的RPC连接。
//{@code RegisteredRpcConnection}实现注册并获取目标网关。
//<p>注册使您能够访问成功注册后就可以完成的将来。 RPC连接可以关闭，例如当它尝试注册的目标失去领导者状态时。
