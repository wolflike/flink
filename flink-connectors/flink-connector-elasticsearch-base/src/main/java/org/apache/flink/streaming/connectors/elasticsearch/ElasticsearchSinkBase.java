...
public abstract class ElasticsearchSinkBase<T, C extends AutoCloseable> extends RichSinkFunction<T> implements CheckpointedFunction {
	...
	//通过{@link ElasticsearchSinkFunction}提供给用户以添加{@link ActionRequest ActionRequests}。
	private transient RequestIndexer requestIndexer;

	/** Provided to the {@link ActionRequestFailureHandler} to allow users to re-index failed requests. */
	//提供给{@link ActionRequestFailureHandler}，以允许用户重新索引失败的请求。
	...
	//大容量处理器，用于缓冲和向使用客户端创建的Elasticsearch发送请求。
	...
	public void open(Configuration parameters) throws Exception {
		...
		//在构建BulkProcessor的时候
		//先创建BulkProcessor.Builder
		//在创建BulkProcessor.Builder的时候需要两个参数client和listener
		//用这两个参数构建一个函数对象，当client去请求的时候，listener可以监听到，可以做各种事
		//最后build BulkProcessor的时候就可以把函数对象传给他
		//这个函数对象是实际的执行请求逻辑

		//所以整个sink逻辑如下：
		//先创建一个ActionRequest add到requestIndexer
		//requestIndexer继续add到bulkProcessor
		//bulkProcessor会有一个BulkRequest（维护一个request的list）
		//bulkProcessor继续add到bulkRequest（这个bulkRequest是通过Supplier函数对象体直接new出来的，每次处理完一个bulkrequest对象就通过
		//supplier对象再次获取）
		//bulkProcessor在构造函数初始化的时候就开始刷新任务了，
		//只不过是以一定间隔时间开始startFlushTask
		//startFlushTask的最终核心代码调用了上面提到的函数对象
