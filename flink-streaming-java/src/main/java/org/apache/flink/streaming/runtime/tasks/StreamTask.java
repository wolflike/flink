...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
	 ...
	 */
	//具有生命周期方法的通用计时器服务接口。
	//内部{@link TimerService}用于定义当前处理时间
	//（默认= {@code System.currentTimeMillis（）}）和
	// 注册计时器以用于将来要执行的任务。
	...
	private void beforeInvoke() throws Exception {
		...
		//创建状态后端，我们来看看是如何创建后端的
		//就是创建四大状态后端之一
		stateBackend = createStateBackend();
		//用状态后端获取检查点存储
		//看了fs的存储，就是包括一堆path和filesystem类
		checkpointStorage = stateBackend.createCheckpointStorage(getEnvironment().getJobID());

		// if the clock is not already set, then assign a default TimeServiceProvider
		//如果尚未设置时钟，则分配默认的TimeServiceProvider
		if (timerService == null) {
			//这个是用来创建线程的
			//线程工厂使用给定名称创建线程，将它们与给定线程组关联，并将它们设置为守护程序模式。
		...
		//重点来看一下OperatorChain是如何生成的
		//第一个参数是StreamTask，主要是用这个对象的environment参数获取一些配置
		...
		actionExecutor.runThrowing(() -> {
			...
			//以下两个操作均受锁保护，以便在initializeState（）注册计时器
			//在调用open（）之前触发）的情况下避免出现竞争情况。

			//这个open（）是AbstractRichFunction的生命周期函数
	 ...
	 *
	 * @return
	 * 可以看到顶层类都会把要进行checkpoint任务直接送进mailbox
	 */
	@Override
	...
	private boolean performCheckpoint(
		...
		if (isRunning) {
			actionExecutor.runThrowing(() -> {
				...
				//准备检查点，允许operator进行一些预屏障工作。
				//  在一般情况下，屏障前的工作应该什么都不做或尽量少做。

				//在做屏障前，先快照一下checkpoint
				...
				//拍摄状态快照。 这在很大程度上应该是异步的，以免影响流拓扑的进度
				//这里才是保存状态啊，最终会调用checkpointedFunction接口的snapshot方法来保存状态
	 ...
	 */
	//执行{@link StreamOperator＃initializeState（）}，
	//然后执行此{@link StreamTask}链中每个运算符的{@link StreamOperator＃open（）}。
	//状态初始化和开放从链中的<b> tail到head </ b>运算符发生，
	//与{@link StreamOperator＃close（）}发生在<b>头到尾</ b>相反（请参阅{@link＃ closeAllOperators（）}。
	...
	private StateBackend createStateBackend() throws Exception {
		//从配置中通过classloader获取到StateBackend
